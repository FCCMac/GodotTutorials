...
var controller_velocity: Vector3 = Vector3(0, 0, 0)
...
func _ready() -> void:
	...
	grab_area = $Area
	grab_raycast = $Grab_Cast
	grab_pos_node = $Grab_Pos
	
	grab_mode = "AREA"
	grab_raycast.visible = false
	
	$Sleep_Area.connect("body_entered", self, "sleep_area_entered")
	$Sleep_Area.connect("body_exited", self, "sleep_area_exited")
	
	hand_mesh = $Hand
	hand_pickup_drop_sound = $AudioStreamPlayer3D
